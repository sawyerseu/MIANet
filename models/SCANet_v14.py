...
class SCANet(nn.Module):
    def __init__(self, load_weights=False):
        ...
        self.front_feat = [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 256, 'M', 512, 512, 512, 512, 'M', 512, 512, 512,
                           512]
        self.front1 = make_layers(self.front_feat)  # VGG 16
        self.front2 = make_layers(self.front_feat)  # VGG 16

        self.alpha = torch.nn.Parameter(torch.FloatTensor(1).uniform_(0, 1), requires_grad=True)
    ...
    def forward(self, x):
        ...
        #x_mul = x1 * x2
        x_gating = self.alpha * x1 + (1 - self.alpha) * x2
        out = self.reg2(x_gating)
