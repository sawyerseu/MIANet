...
class SCANet(nn.Module):
    def __init__(self, pretrained=False, ratio=1):
        ...
        self.comc = nn.Conv2d(512*4, 512, 1)
        ...
        self.dila4 = nn.Conv2d(512, 256, 3, stride=1, padding=3, dilation=3)

        self.backend_feat = [int(1024), int(512 * ratio), int(512 * ratio), int(256 * ratio), int(128 * ratio),
                             64]
        self.backend = make_layers(self.backend_feat, in_channels=int(1024), d_rate=2)
    ...
    def forward(self, RGBT):
        ...
        o1_first, o2_first = self.block1(x1, x2)               #1 44 128 128
        o1_second, o2_second = self.block2(o1_first, o2_first) # 1 89 64 64
        o1_thir, o2_thir = self.block3(o1_second, o2_second)   #1 179 32 32
        o1_four, o2_four = self.block4(o1_thir, o2_thir)       #1 358 32 32
        ...
        back_feat4 = F.interpolate(back_feat4, scale_factor=1, mode='bilinear')

        # 1-Addition
        #back_add1 = 0.0005 * (back_feat1) + 0.005 * (back_feat2) + 0.05 * (back_feat3) + (back_feat4) # 1*512*30*30
        ...
        back_dila4 = self.dila4(back_add1)

        back_add2 = torch.cat((back_dila1, back_dila2, back_dila3, back_dila4), 1) #1 1024 128 128
