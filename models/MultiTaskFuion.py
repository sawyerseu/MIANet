...
class FusionModel(nn.Module):
    def __init__(self, num_classes, ratio=0.6):
        ...
        self.class_out = nn.Softmax(dim=1)
    ...
    def forward(self, RGBT):
        ...
        x1 = F.interpolate(x, scale_factor=2, mode='bilinear' )

        #output 1 densitymap
        x_denmap = self.reg_layer(x1)

        # output 2 density level
        x_den_class = self.class_layer(x1)
        x_den_class = x_den_class.view(x_den_class.size(0), -1)
        x_num_of_class = self.classifier(x_den_class)
        x_num_of_class = self.class_out(x_num_of_class)

        return torch.abs(x_denmap), x_num_of_class
...
if __name__ == "__main__":
    ...
    #测试
    x1 = torch.rand((1, 3, 64, 64)).to(device)  #RGB
    x2 = torch.rand((1, 3, 64, 64)).to(device)   #T
    x = [x1,x2]
    #print(x.shape)
    #x = x.to(device)

    model = FusionModel(5).to(device)
    o1, o2 = model(x)
    print(o2)
    print(o1.shape)
