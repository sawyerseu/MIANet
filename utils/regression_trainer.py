...
L1_loss = nn.L1Loss()
#huber_loss = nn.HuberLoss(delta=1.0).to('gpu')
    ...
    def train_eopch(self):
        ...
        for step, (inputs, points, st_sizes) in enumerate(self.dataloaders['train']):
            ...
            with torch.set_grad_enabled(True):
                #print(inputs[0].shape)  #torch.Size([1, 1, 256, 256])
                outputs = self.model(inputs)
                #print(outputs.shape)  #torch.Size([1, 1, 32, 32])
                prob_list = self.post_prob(points, st_sizes)
                #print(prob_list[0][1].shape)
                loss = self.criterion(prob_list, outputs)
                #print(len(prob_list)) #1
                #print((outputs.shape))  # [1, 1, 32, 32]


                '''  #  wsy 2023.2.3
                for idx, prob in enumerate(prob_list):
                    if prob is None:
                        prob_loss = torch.zeros((outputs.shape[0], outputs.shape[1], outputs.shape[2], outputs.shape[3]), dtype=torch.float32, device=self.device)
                    else:
                        prob_loss = prob_list[0][1].reshape(outputs.shape[0], outputs.shape[1], outputs.shape[2], outputs.shape[3])
                loss_mse = mse_loss(prob_loss, outputs)
                loss = loss + 0.0001 * loss_mse
                '''
                #2023.03.15
                if type(inputs) == list:
                    N = inputs[0].size(0)
                    ...
                    N = inputs.size(0)
                pre_count = torch.sum(outputs.view(N, -1), dim=1).to(self.device)
                gd_count = torch.from_numpy(gd_count).to(self.device)
                #print(type(pre_count))
                #print(type(gd_count))
                loss2 = L1_loss(pre_count, gd_count)
